...
- feat: two optional, additive fields on `HarnessInfo`. `harness.series` (`{ id, index, count }`, new type `HarnessSeries`) marks a run of a consecutive series a host ran on one device with the same settings, one fresh page load per run. `harness.coldStart: 'provider-caches-cleared'` marks a run that started right after the host page deleted its providers' model caches and found them empty; it does not claim a fresh browser profile (the HTTP disk cache and browser-wide models are out of a page's reach). `validateRunShape` accepts both and rejects malformed values (`series.id` 1 to 64 characters, `series.count` an integer from 1 to 1000, `series.index` an integer from 1 to `count`; `coldStart` only `'provider-caches-cleared'`). The digest covers them like the rest of `harness`. No schema version change and no protocol change: nothing measured changes.
- feat: `runsToRunsCSV` exports `seriesId`, `seriesIndex`, `seriesCount` and `coldStart` after `validationFlags`, before the `rv_*` columns (which move four places right); empty on runs without the fields. Every other column keeps its name and value.
- chore: `pnpm test` in the package runs its tests through the workspace Vitest config (`vitest run --root ../.. packages/bench`); it used to exit with "No test files found".
