...
describe('DexieStorage', () => {
  ...
  describe('vector payload types (SQ8/PQ compression)', () => {
    ...
    it('reads legacy records (raw ArrayBuffer of f32 data) as Float32Array', async () => {
      ...
      // injected at the Dexie table level (there is no public API that can
      // produce a legacy record); the assertions go through the public
      // getVector()/getAllVectors() API.
      const legacy = new Float32Array([4.5, -1.25]);
      const internalDb = (
        storage as unknown as {
          db: {
            vectors: {
              put(record: { id: string; collectionId: string; vector: ArrayBuffer }): Promise<unknown>;
            };
          };
        }
      ).db;
      await internalDb.vectors.put({
        id: 'v-legacy',
        collectionId: 'default',
  ...
  describe('createVectorDB integration', () => {
    ...
    it('round-trips SQ8-compressed vectors (Uint8Array payloads) through this adapter', async () => {
      const { createVectorDB } = await import('@localmode/core');
      const customStorage = new DexieStorage({ name: `integration-sq8-${Date.now()}` });
