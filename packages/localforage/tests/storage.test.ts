/**
 * @fileoverview Tests for LocalForageStorage — mirrors core/tests/storage.test.ts
 */
...
describe('LocalForageStorage', () => {
  ...
  describe('createVectorDB integration', () => {
    it('works as custom storage with createVectorDB', async () => {
      ...
      const customStorage = new LocalForageStorage({ name: `integration-${Date.now()}-${testCounter++}` });
