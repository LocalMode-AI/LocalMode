/**
 * @file ui.tsx
 * @description Reusable UI components for the semantic-search application
 */
import { ButtonHTMLAttributes, HTMLAttributes, SelectHTMLAttributes, forwardRef, ReactNode } from 'react';
import { Loader2 } from 'lucide-react';
import type { SearchLatency } from '../_lib/types';
import { formatLatency } from '../_lib/utils';
...
/** Styled select dropdown with daisyUI classes */
export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ options, className, ...props }, ref) => (
    <select
      ref={ref}
      className={cn('select select-bordered select-sm w-full bg-black/20 border-poster-border/20 text-poster-text-main', className)}
      {...props}
    >
      {options.map((opt) => (
...
// ============================================================================
// LatencyBadge - Search latency indicator
// ============================================================================

/** Props for the LatencyBadge component */
interface LatencyBadgeProps {
  /** Latency measurement from the search */
  latency: SearchLatency;
  /** Additional CSS classes */
  className?: string;
}

/** Displays search latency in ms with GPU-aware color */
export function LatencyBadge({ latency, className }: LatencyBadgeProps) {
  return (
    <span
      className={cn(
        'text-[11px] font-medium tabular-nums',
        latency.gpuEnabled ? 'text-poster-accent-teal' : 'text-poster-text-sub/60',
        className
      )}
    >
      {formatLatency(latency.queryMs)}
    </span>
  );
}
