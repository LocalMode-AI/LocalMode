...
**Curated catalog**: 18 curated models from SmolLM2 135M (70MB) to Holo2 8B (5.1GB), spanning Q4_K_M text models and two vision-language models (Holo2 4B and 8B for UI grounding). But the real story is the open ecosystem: wllama accepts any GGUF URL, which means access to the 160,000+ GGUF models on HuggingFace Hub. Any model that bartowski, TheBloke, or the original authors have quantized to GGUF is one URL away.
...
| **Primary runtime** | WebGPU (GPU) | WebGPU + WASM fallback | WASM (CPU) | WebGPU (Gemma 4); WebGPU or CPU (Qwen3 0.6B) |
| **Curated models** | 32 | 16 | 18 (+ 160K+ via any GGUF URL) | 3 (Gemma 4 E2B/E4B, Qwen3 0.6B) |
| **Vision models** | 1 (Phi 3.5 Vision) | 5 (Qwen3.5 family, Gemma 4 E2B/E4B) | 2 (Holo2 4B/8B, UI grounding) | None (JS API is text-only) |
| **Custom models** | No (MLC-compiled only) | Yes (any ONNX on HF Hub) | Yes (any GGUF on HF Hub) | Yes (any `.litertlm` URL) |
| **Size range** | 78MB -- 5.06GB | 120MB -- 3GB | 70MB -- 5.1GB | 614MB -- 3.0GB |
| **Inference speed** | 60-100 tok/s | 40-60 tok/s (WebGPU) | 5-15 tok/s (multi-thread) | Not yet benchmarked |
...
| **WebGPU required** | Yes | No (preferred, not required) | No | Yes for Gemma 4; no for Qwen3 0.6B |
