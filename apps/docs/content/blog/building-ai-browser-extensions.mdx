...
Instead, download models on first use and cache them in the browser's Cache API. This is exactly how LocalMode works by default -- Transformers.js and WebLLM both cache downloaded models in the browser's storage.
...
The provider packages (`@localmode/transformers`, `@localmode/webllm`, `@localmode/wllama`) download model files from HuggingFace Hub on first use, then cache them permanently in the browser cache. After that initial download, the extension works fully offline.
