...
**NER**: `bert-base-NER` achieves **91.3% F1** on the CoNLL-2003 test set. GPT-4o achieves approximately 94-97% with careful prompting. The local model reaches 95-98% of cloud quality at 30-100ms latency (vs. 500-3000ms for GPT-4o).

**QA**: `distilbert-squad` achieves **87.1 F1** on SQuAD v1.1 dev set. Human performance is 91.2. GPT-4o reaches approximately 91-95 F1. The local model delivers 92-95% of cloud quality for extractive QA over short documents - at 20-100ms versus 500-3000ms.
...
The headline result: **Qwen3.5-4B** (ONNX, Feb 2026) scores **88.8% on MMLU-Redux** in thinking mode - within 0.1% of GPT-4o's 88.7% on MMLU. (Note: MMLU and MMLU-Redux are related but distinct benchmarks; this is a close comparison, not an exact apples-to-apples match.) On math reasoning, the Qwen3 family dramatically outperforms GPT-4o: Qwen3-8B solves 73.8% of AIME 2024 problems (thinking mode, per the Qwen3 Technical Report) compared to GPT-4o's approximately 9% (per the DeepSeek-R1 paper).

| | Qwen3.5-4B (ONNX) | Qwen3-4B (MLC) | Qwen3-8B (MLC) | DeepSeek-R1-Distill-7B (MLC) | GPT-4o (cloud) |
|---|---|---|---|---|---|
| Knowledge (MMLU-Redux) | **88.8%** (thinking) | 84.2% | ~79%* | N/A | 88.7% (MMLU) |
| Math reasoning (AIME 2024) | N/A | ~66%* | **73.8%** | 55.5% | ~9% |
...
For most users, **Qwen3.5-4B** (~2.4GB MLC download via WebLLM, or larger via ONNX) is the new practical sweet spot - it runs on most modern laptops via the Transformers.js v4 or WebLLM provider with WebGPU acceleration and achieves the highest knowledge benchmark scores of any local browser model. For pure math reasoning, users with 6GB+ GPU VRAM can load **Qwen3-8B** via WebLLM for the best AIME/MATH-500 results. For universal browser support without WebGPU, **wllama** runs any of 160,000+ GGUF models via WASM.
...
- [dslim/bert-base-NER model card](https://huggingface.co/dslim/bert-base-NER) - CoNLL-2003 F1 91.3% (test set)
...
- [DeepSeek-R1 paper (arXiv:2501.12948)](https://arxiv.org/abs/2501.12948) - Distilled model reasoning benchmarks: R1-Distill-Qwen-7B AIME 2024 55.5%, MATH-500 92.8%; GPT-4o AIME 2024 9.3%
...
**Pricing Sources:**
- [OpenAI API Pricing](https://openai.com/api/pricing/) - GPT-4o ($2.50/$10 per 1M tokens in/out), embeddings, Whisper ($0.006/min), TTS ($15/$30 per 1M chars)
