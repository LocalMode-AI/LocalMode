...
faq:
  - question: "How do I decide which AI tasks to run locally vs in the cloud?"
    answer: "Run locally: embeddings, classification, NER, reranking, speech-to-text, image classification, and simple generation. Run in cloud: complex reasoning, creative writing requiring frontier quality, tasks needing 100K+ token context, and real-time translation for rare language pairs."
  - question: "What is the SaaS local mode toggle pattern?"
    answer: "It is a product pattern where your SaaS app offers a toggle between local and cloud processing. When set to local, all inference runs in the browser via LocalMode. When set to cloud, your backend handles inference. This gives privacy-conscious users control while maintaining cloud capability for others."
  - question: "How much does local-first architecture save at scale?"
    answer: "A case study at localmode.dev/blog/cut-ai-api-bill-200k shows a 100K-user app saving $212K/year by moving embeddings, classification, and reranking from OpenAI/Cohere to LocalMode. The savings come from eliminating per-token costs for high-volume, low-complexity tasks."
---
