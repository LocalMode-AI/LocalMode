...
faq:
  - question: "Does LocalMode work with Vue, Svelte, and Angular?"
    answer: "Yes. The core packages (@localmode/core and provider packages like @localmode/transformers) are plain TypeScript with zero framework dependencies. They export async functions that work in Vue 3, Svelte 5, Angular, Solid, Qwik, Lit, or vanilla JavaScript -- 14 out of 15 LocalMode packages are framework-agnostic."
  - question: "Is LocalMode a React-only library?"
    answer: "No. @localmode/react is the only package with a React dependency, and it is a convenience layer that wraps the same core functions in React hooks. The core package has zero runtime dependencies. All AI features work identically in any framework."
  - question: "How do I integrate LocalMode with a Vite-based framework?"
    answer: "For Vite-based frameworks (Vue, Svelte, SvelteKit), you may need to exclude the transformers dependency from pre-bundling by adding '@huggingface/transformers' to the optimizeDeps.exclude array in vite.config.ts."
  - question: "What LocalMode features are available in non-React frameworks?"
    answer: "Everything: embeddings, vector search, LLM chat with streaming, classification, NER, vision tasks, audio transcription, TTS, translation, summarization, document QA, RAG pipelines, encryption, PII redaction, and agents. All are accessible through async functions and options objects."
---

import { Callout } from 'fumadocs-ui/components/callout';

Most of the LocalMode examples you see online are written in React. That is because the blocks app at [localmode.ai](https://localmode.ai) is a Next.js application, and because `@localmode/react` ships 59 purpose-built hooks that make React integration especially smooth.
...
| `@localmode/webllm` | LLM chat via WebGPU (32 models) | Yes | Yes | Yes | Yes | Yes |
...
| `@localmode/react` | React hooks (61 hooks) | **Yes** | No | No | No | No |

The pattern is clear: 14 out of 15 packages work with any framework. The one exception exists specifically to provide idiomatic React integration.
...
Models download from HuggingFace Hub on first use and cache in the browser cache. Show a loading indicator during this one-time download:
