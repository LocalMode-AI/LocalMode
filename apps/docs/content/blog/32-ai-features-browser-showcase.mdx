---
title: "The 34 AI Features in Our Open-Source Blocks Gallery - All Running in Your Browser Right Now"
description: "A visual tour of every demo app at localmode.ai: LLM chat with four inference backends, RAG pipelines, real-time object detection, voice transcription, GGUF model inspection, agentic reasoning, and 26 more - all running locally with zero API keys."
author: LocalMode
date: 2026-01-29
dateModified: "2026-07-04"
faq:
  - question: "How many AI demo apps are available at localmode.ai and what do they cover?"
    answer: "There are 34 fully functional AI applications covering LLM chat with four inference backends, RAG pipelines, real-time object detection, voice transcription, GGUF model inspection, agentic reasoning, privacy tools, and more. All run entirely in the browser with zero API keys."
  - question: "What inference backends does LocalMode support?"
    answer: "Four backends are available: WebLLM (WebGPU-accelerated via MLC), Transformers.js (ONNX via WebGPU/WASM), wllama (GGUF models via llama.cpp WASM with access to 180,000+ models), and LiteRT (Google's on-device engine). The LLM chat app surfaces 76 models across all four backends."
  - question: "What is the range of model sizes used in the LocalMode apps?"
    answer: "Models range from 0 MB (Chrome AI, device detection tools) to 5.41 GB (8B-parameter LLMs). The smallest ML model is MobileBERT MNLI at 25 MB for email classification. Every model downloads once from HuggingFace, caches in the browser, and loads from disk on subsequent visits."
  - question: "Can I run the LocalMode blocks gallery locally on my own machine?"
    answer: "Yes. Clone the open-source repo, run pnpm install, then pnpm --filter ui dev. The blocks gallery is available at localhost:3000/blocks, with every feature consolidated into 11 installable blocks. Each block is a self-contained reference implementation in its own directory under apps/ui/src/app/blocks/."
---

import { Callout } from 'fumadocs-ui/components/callout';

Open [localmode.ai](https://localmode.ai) in Chrome or Edge — it now lands on the [/blocks gallery](https://localmode.ai/blocks), where the 34 AI features cataloged below run live, consolidated into 11 fully functional, installable blocks. Every one of them runs entirely in your browser tab. No backend server. No API key. No data leaves your device. Close the tab and the computation stops - there is nothing to shut down, no bill to reconcile, no logs on a server you do not control.

This is not a collection of toy demos. These are complete applications - with file upload, drag-and-drop, batch processing, export to CSV/JSON/SRT, real-time streaming, and persistent IndexedDB storage - built on the same [`@localmode`](https://localmode.dev) packages you would use in production. The gallery exists so you can see exactly what local-first AI looks like before you write a single line of code.
...
**Models:** 76 models across four backends (Llama 3.2, Qwen 3, Phi 3.5, Mistral, DeepSeek R1, Gemma 4, and more) | **Size:** 78 MB -- 5.41 GB | [Try LLM Chat](https://localmode.ai/blocks/chat)
...
**Models:** Any GGUF model on HuggingFace + 30 curated defaults (25 language + 3 embedding + 2 reranker) | **Size:** 35 MB -- 5.41 GB | [Try GGUF Explorer](https://localmode.ai/blocks/device/gguf-explorer)
...
**Model:** Qwen 3 1.7B (WebLLM) | **Size:** 1.1 GB | [Try Research Agent](https://localmode.ai/blocks/agents/research-agent)
...
**Models:** Qwen 3 1.7B through Llama 3.1 8B (WebLLM) | **Size:** 1.1 GB -- 4.9 GB | [Try Data Extractor](https://localmode.ai/blocks/agents/data-extractor)
...
**Models:** BGE Small (33 MB) + MS MARCO Reranker (22 MB) + optional Llama 3.2 1B | [Try PDF Search](https://localmode.ai/blocks/knowledge/semantic-search)
...
**Model:** BGE Small | **Size:** 33 MB | [Try Semantic Search](https://localmode.ai/blocks/knowledge/semantic-search)
...
**Models:** BGE Small (33 MB) + Qwen 3 1.7B (1.1 GB) | [Try LangChain RAG](https://localmode.ai/blocks/knowledge/rag-chat)
...
**Model:** BGE Small | **Size:** 33 MB | [Try Data Migrator](https://localmode.ai/blocks/knowledge/vector-data-manager)
...
Classify reviews, support tickets, or social mentions as positive or negative. Paste a single text or batch-process hundreds at once. The app shows a statistics dashboard with distribution charts and lets you export results to CSV. One of the smallest model downloads in the gallery at 67 MB.

**Model:** DistilBERT SST-2 | **Size:** 67 MB | [Try Sentiment Analyzer](https://localmode.ai/blocks/text-insights/sentiment-analyzer)
...
**Model:** MobileBERT MNLI | **Size:** 25 MB | [Try Email Classifier](https://localmode.ai/blocks/text-insights/text-classifier)
...
**Model:** DistilBART CNN | **Size:** 300 MB | [Try Text Summarizer](https://localmode.ai/blocks/writing-tools/summarize)
...
**Model:** DistilBERT SQuAD | **Size:** 100 MB | [Try Q&A Bot](https://localmode.ai/blocks/knowledge/document-qa)
...
**Models:** Opus MT (EN-DE, EN-FR, EN-ES, and more) | **Size:** 100 -- 300 MB per language pair | [Try Translator](https://localmode.ai/blocks/writing-tools/translate)
...
**Model:** ModernBERT Base | **Size:** 150 MB | [Try Smart Autocomplete](https://localmode.ai/blocks/writing-tools/complete)
...
**Models:** Chrome AI (0 MB) with DistilBART CNN fallback (300 MB) | [Try Smart Writer](https://localmode.ai/blocks/writing-tools/write)
...
**Model:** Donut Base (DocVQA) | **Size:** ~800 MB | [Try Invoice Q&A](https://localmode.ai/blocks/knowledge/document-qa)
...
**Model:** RMBG-1.4 | **Size:** 170 MB | [Try Background Remover](https://localmode.ai/blocks/image-studio/background-remover)
...
**Model:** SigLIP Base | **Size:** 400 MB | [Try Smart Gallery](https://localmode.ai/blocks/photo/smart-gallery)
...
**Model:** SigLIP Base | **Size:** 400 MB | [Try Product Search](https://localmode.ai/blocks/photo/image-search)
...
**Model:** ViT-GPT2 Image Captioning | **Size:** ~230 MB | [Try Image Captioner](https://localmode.ai/blocks/image-studio/image-captioner)
...
**Model:** TrOCR Small Printed | **Size:** 10 -- 50 MB | [Try OCR Scanner](https://localmode.ai/blocks/knowledge/semantic-search)
...
**Model:** DETR ResNet-50 | **Size:** ~42 MB | [Try Object Detector](https://localmode.ai/blocks/vision/object-detector)
...
**Model:** Swin2SR Lightweight x2 | **Size:** 50 MB | [Try Photo Enhancer](https://localmode.ai/blocks/image-studio/image-enhancer)
...
**Model:** DINOv3 Small | **Size:** 86 MB | [Try Duplicate Finder](https://localmode.ai/blocks/photo/duplicate-finder)
...
**Model:** CLIP ViT Base Patch32 | **Size:** ~350 MB | [Try Cross-Modal Search](https://localmode.ai/blocks/photo/image-search)
...
**Models:** Moonshine Tiny (50 MB) + BGE Small (33 MB) | [Try Voice Notes](https://localmode.ai/blocks/audio/voice-notes)
...
**Models:** Moonshine Base (~237 MB) + DistilBART CNN (~200 MB) | [Try Meeting Assistant](https://localmode.ai/blocks/audio/meeting-assistant)
...
**Model:** Kokoro-82M | **Size:** ~86 MB | [Try Audiobook Creator](https://localmode.ai/blocks/audio/audiobook-reader)
...
**Model:** BERT Base NER | **Size:** 110 MB | [Try Document Redactor](https://localmode.ai/blocks/privacy/pii-redactor)
...
**Model:** BGE Small (for semantic search) | **Size:** 33 MB | [Try Encrypted Vault](https://localmode.ai/blocks/privacy/encrypted-vault)
...
**Models:** None (0 MB) | [Try Model Advisor](https://localmode.ai/blocks/device/model-advisor)
...
**Model:** DistilBERT SST-2 (or MobileBERT MNLI) | **Size:** 25 -- 67 MB | [Try Model Evaluator](https://localmode.ai/blocks/text-insights/model-evaluator)
...
**Models:** MediaPipe task bundles (2--10 MB each) | [Try MediaPipe Studio](https://localmode.ai/blocks/vision/live-tracker)
...
**Model:** Kokoro-82M | **Size:** ~86 MB | [Try Voice Studio](https://localmode.ai/blocks/audio/voice-explorer)
...
- **`@localmode/react`** provides 59 React hooks that manage loading states, cancellation, error handling, and streaming for every operation.
...
The blocks gallery itself is a Next.js 16 application with React 19 and shadcn/ui. Each block is fully self-contained, and you can read the source of any block as a standalone reference implementation.
...
Every model is downloaded once from HuggingFace and cached in the browser's Cache API or OPFS. Subsequent visits load from disk in seconds.
...
Open `http://localhost:3000/blocks` and every feature is available locally. You can also read the source of any block at `apps/ui/src/app/blocks/{category}/{block}/` - each one is a self-contained reference implementation with its own components and hooks.

---

## Methodology

All feature names, counts, model IDs, and model sizes were verified against the block sources under `apps/ui/src/app/blocks/` and the model catalogs in `packages/`. Package counts were verified against the `packages/` directory. React hook counts were verified against `packages/react/src/index.ts`. WebLLM and wllama model counts were verified against `packages/webllm/src/models.ts` and `packages/wllama/src/models.ts` respectively. The GGUF model count was verified against [huggingface.co/models?library=gguf](https://huggingface.co/models?library=gguf) (180,000+ as of May 2026). Moonshine Base size was cross-referenced against the LocalMode model catalog (~237 MB).
...
- [Xenova/detr-resnet-50 model card](https://huggingface.co/Xenova/detr-resnet-50) -- object detection model used in Object Detector app
...
- [Xenova/donut-base-finetuned-docvqa model card](https://huggingface.co/Xenova/donut-base-finetuned-docvqa) -- model used in Invoice Q&A app
...
<Callout type="info" title="Try it yourself">
  Visit the [/blocks gallery](https://localmode.ai/blocks) to try these features as 11 installable AI blocks running entirely in your browser. No sign-up, no API keys, no data leaves your device.
