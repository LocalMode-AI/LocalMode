...
faq:
  - question: "Does Safari on macOS support WebGPU for AI inference?"
    answer: "WebGPU is enabled by default from Safari 26 (macOS Tahoe) using the Metal backend. In Safari 17.4-25, WebGPU was available only behind a developer feature flag, which is not suitable for production end-users. For older Safari versions, LocalMode uses WASM-based inference via wllama or Transformers.js."
  - question: "Does LocalMode work in Safari Private Browsing on macOS?"
    answer: "AI inference works normally, but IndexedDB is ephemeral in Safari Private Browsing — IndexedDB works but uses in-memory storage that is cleared when the session ends. This means models and vectors are not persisted between sessions. LocalMode detects this with a probe write and falls back to MemoryStorage. Users should use normal browsing mode for the best experience."
  - question: "What is the minimum Safari version needed for LocalMode?"
    answer: "WebAssembly works from Safari 11+. WASM SIMD requires Safari 16.4+. Module workers require Safari 15+. Web Locks and BroadcastChannel require Safari 15.4+. WebGPU enabled by default requires Safari 26+."
  - question: "Can Safari's Intelligent Tracking Prevention block model downloads?"
    answer: "Safari's ITP may affect cross-origin model CDN requests in some configurations. If model downloads fail, verify that the CDN domain is not being blocked. Setting appropriate CORS headers on the model CDN and using the crossorigin attribute on requests can help mitigate this."
  - question: "What happens if WebGPU is not available on Safari?"
    answer: "LocalMode falls back to WASM-based providers. Use wllama for LLM inference (5-20 tokens per second) and Transformers.js for non-LLM tasks like embeddings, classification, vision, and audio. All features work without WebGPU, only LLM inference speed is reduced."
---
...
| --- | --- | --- |
| **WebGPU** | Safari 17.4+ (behind flag), Safari 26+ (default) | WebGPU behind feature flag in Safari 17.4-25. Enabled by default from Safari 26 (macOS Tahoe). Metal backend. |
| **WebAssembly** | Yes (Safari 11+) | Full WASM support. SIMD support from Safari 16.4. |
| **IndexedDB** | Yes (normal mode) | Full support in normal browsing. Ephemeral in Private Browsing (in-memory, cleared when session ends) - use MemoryStorage fallback. |
...
// Handle Safari Private Browsing - IndexedDB is ephemeral (in-memory, cleared on session end)
...
Private Browsing makes IndexedDB ephemeral - it works but uses in-memory storage cleared when the session ends, so models and vectors are not persisted. Safari 18 WebGPU has occasional shader compilation issues with certain model architectures. Safari's ITP (Intelligent Tracking Prevention) may affect cross-origin model CDN requests in some configurations.
