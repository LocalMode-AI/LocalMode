...
All private browsing modes restrict storage persistence. In Chrome Incognito and Edge InPrivate, IndexedDB is backed by in-memory storage only (no disk writes) with a storage quota capped at roughly 5% of total disk space. In Safari Private Browsing, IndexedDB is available but ephemeral - data never reaches disk and vanishes when the private window closes. Firefox Private Browsing (since Firefox 115) uses encrypted on-disk storage that is wiped at session end; versions before Firefox 115 blocked IndexedDB entirely. In all cases, cached models are unavailable across private sessions and must be re-downloaded - regardless of where the provider persists them (the Cache API for Transformers.js and LiteRT, OPFS for wllama, IndexedDB for `createModelLoader`). The MemoryStorage fallback means all vectors and model caches are lost when the tab closes. There is no workaround for persistent storage in any private browsing mode.
