...
faq:
  - question: "Why are small 30MB models better for browser AI than large LLMs?"
    answer: "The relationship between model size and quality is logarithmic, not linear. Going from 5MB to 50MB buys enormous quality gains, but going from 500MB to 5GB yields diminishing returns. A 33MB embedding model matches OpenAI at 99.8% quality, and a 23MB reranker replaces a $73,000/year Cohere bill."
  - question: "What are the best small models for browser AI under 100MB?"
    answer: "The sweet spot includes bge-small-en-v1.5 (33MB, embeddings, 99.8% of OpenAI), ms-marco-MiniLM-L-6-v2 (23MB, reranking, 87-93% of Cohere), mobilebert-mnli (~21MB, zero-shot classification, 92-95% of GPT-5), distilbert-sst2 (~67MB, sentiment, 95%+ of cloud), and moonshine-tiny (~27-50MB, speech-to-text)."
  - question: "How fast do small browser AI models load?"
    answer: "Models in the 4-100MB range load in under 10 seconds on a mobile connection. The 23MB embedding model loads in about 2 seconds on a typical broadband connection, and the 33MB BGE-small loads in roughly 3 seconds. These sizes are practical even for phones with 4GB of RAM."
  - question: "When should I use a large LLM instead of small specialized models?"
    answer: "Use large LLMs (1-4GB) only when you need open-ended text generation, complex reasoning, or multi-step problem solving. For specific tasks like embeddings, classification, NER, reranking, QA, and transcription, small purpose-built models deliver 85-99% of cloud quality at a fraction of the size and latency."
---
...
All three models cache in the browser after the first download. On subsequent visits, they load from local cache in milliseconds. Total combined download: approximately 150MB. Total API cost: $0. Total data sent to external servers: zero bytes.
...
- [BAAI/bge-small-en-v1.5 model card](https://huggingface.co/BAAI/bge-small-en-v1.5) -- MTEB average 62.17, 384 dimensions, 33.4M parameters
- [BAAI/bge-base-en-v1.5 model card](https://huggingface.co/BAAI/bge-base-en-v1.5) -- MTEB average 63.55, 768 dimensions, ~110MB ONNX INT8
- [BAAI/bge-large-en-v1.5 model card](https://huggingface.co/BAAI/bge-large-en-v1.5) -- MTEB average 64.23, 1024 dimensions
- [Snowflake/snowflake-arctic-embed-xs model card](https://huggingface.co/Snowflake/snowflake-arctic-embed-xs) -- 22M parameters, 384 dimensions, MTEB Retrieval NDCG@10 50.15, state-of-the-art at size tier
- [sentence-transformers/all-mpnet-base-v2 model card](https://huggingface.co/sentence-transformers/all-mpnet-base-v2) -- MTEB average 57.78, 768 dimensions, ~420MB
...
- [dslim/bert-base-NER model card](https://huggingface.co/dslim/bert-base-NER) -- CoNLL-2003 test F1 92.6%
...
- [UsefulSensors/moonshine-tiny model card](https://huggingface.co/UsefulSensors/moonshine-tiny) -- 27M parameters, LibriSpeech clean WER 4.55, other WER 11.68; 5x faster than Whisper-tiny per arXiv:2410.15608
- [Kokoro-82M model card](https://huggingface.co/hexgrad/Kokoro-82M) -- 82M parameters; LocalMode ships 29 English voices via the ONNX port
- [D-FINE paper / GitHub](https://github.com/Peterande/D-FINE) -- D-FINE-N (nano): 42.8 COCO AP, 4M parameters, 2.12ms T4 latency; ICLR 2025 Spotlight
