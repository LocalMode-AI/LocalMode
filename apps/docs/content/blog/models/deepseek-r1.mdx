...
faq:
  - question: "What is the smallest DeepSeek-R1 model that runs in the browser?"
    answer: "The DeepSeek-R1-Distill-Qwen-1.5B ONNX variant at 500MB runs via Transformers.js and does not require WebGPU. The larger 7B and 8B WebLLM variants (4.18-4.41GB) require WebGPU and are desktop-only in practice."
  - question: "How do DeepSeek-R1 models differ from standard LLMs?"
    answer: "DeepSeek-R1 models naturally produce thinking traces with structured reasoning steps before arriving at an answer. This makes them ideal for tutoring systems, code debugging, and analytical tools where showing work matters."
  - question: "Does DeepSeek-R1 require WebGPU?"
    answer: "The WebLLM variants (7B and 8B) require WebGPU and substantial GPU memory. The smaller 1.5B ONNX variant via Transformers.js can run with WebGPU acceleration where available but also falls back to WASM."
  - question: "Can I run DeepSeek-R1 offline in the browser?"
    answer: "Yes. All DeepSeek-R1 variants work offline after the initial download. Models are cached by the browser via LocalMode's model caching system, so subsequent uses start without any network connection."
---
