...
The `@localmode/transformers` package (version 4.1.3) depends on `@huggingface/transformers` ~4.2.0 (pinned to the 4.2 line, because 4.3.0 breaks language models on WebGPU) and implements 26 model factory methods across 27 implementation files. Each factory returns an object that implements a core interface from `@localmode/core` -- meaning the same `embed()`, `classify()`, `transcribe()`, or `generateText()` function works regardless of which provider produced the model.
...
- `packages/transformers/package.json` - `@huggingface/transformers` ~4.2.0, package version 4.1.3
