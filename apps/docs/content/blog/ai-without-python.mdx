...
- **First load.** Models must be downloaded once (33MB-2GB depending on the task). After that, they are cached by the browser and load from disk in seconds.
