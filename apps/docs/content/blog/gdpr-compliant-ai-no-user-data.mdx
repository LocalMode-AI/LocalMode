...
description: "On-device browser inference eliminates the data processor relationship, cross-border transfers, and DPA negotiations that cloud AI demands. Four technical patterns -- PII redaction, encrypted storage, differential privacy, and local classification -- show how to architect AI features that satisfy GDPR Articles 5, 25, 28, and 35 by design."
author: LocalMode
date: 2026-02-05
dateModified: "2026-07-04"
faq:
  - question: "How does on-device AI inference change GDPR compliance requirements?"
    answer: "When ML inference runs in a WebAssembly sandbox inside the browser, no personal data reaches any third party. This eliminates the data processor relationship (Article 28), removes cross-border transfer obligations (Chapter V), achieves data minimization by architecture (Article 5(1)(c)), and implements privacy by design (Article 25) at the infrastructure level."
  - question: "What are the four technical patterns for GDPR-compliant AI in the browser?"
    answer: "PII detection and redaction before processing (redactPII strips emails, phones, SSNs before embedding), encrypted metadata at rest (AES-GCM encryption via Web Crypto API), differential privacy noise injection on embeddings (dp-middleware adds calibrated noise), and local classification that never transmits user text to any server."
  - question: "Does on-device AI completely eliminate GDPR obligations?"
    answer: "No. On-device inference eliminates server-side data risks and the Article 28 processor relationship, but client-side threats remain: malicious browser extensions, XSS attacks, and compromised npm dependencies. A DPIA may still be recommended for certain processing activities, though its scope is significantly reduced. Consult qualified legal counsel for your specific situation."
  - question: "How does local AI handle the GDPR Article 44 cross-border transfer requirement?"
    answer: "When inference happens in the browser, data does not leave the device, so there is no transfer to assess. This eliminates the need for adequacy decisions, Standard Contractual Clauses, or Binding Corporate Rules that US-based AI API providers require. The model files themselves are public open-weight assets, not personal data."
---
...
The following patterns use real API signatures from `@localmode/core` (version 2.5.0, zero runtime dependencies). Every function shown here runs entirely in the browser.
...
// Create a vector database with encrypted metadata
...
const encryptedDb = wrapVectorDB({
  ...
  middleware: encryptionMiddleware({
    key,
    encryptMetadata: true,   // Encrypt all metadata fields (the default)
    excludeFields: ['type'], // Leave non-sensitive fields for filtering
  }),
});

// Metadata is encrypted before the IndexedDB write, decrypted on read
...
The middleware encrypts metadata -- the patient name and diagnosis above -- but leaves the vector itself in plaintext, because the index has to compute distances over it to store and search it. (Passing `encryptVectors: true` throws a `ValidationError`.) Combine this with Pattern 1 so the embedded text carries no PII signal, and with Pattern 3 so the stored vector is noised. To protect vectors at rest outside the live index, encrypt an exported snapshot with `encryptVector()`.
...
**GDPR relevance**: Encryption at rest is explicitly cited in Article 32(1)(a) as an appropriate technical measure. The [Encrypted Vault block](https://localmode.ai/blocks/privacy/encrypted-vault) demonstrates password-based encryption with PBKDF2 key derivation and lock/unlock lifecycle.
...
// 4. Vector database with encrypted metadata
...
// Pipeline: classify locally, embed with PII redaction + DP noise, store with encrypted metadata
...
- The document's metadata was encrypted with AES-256-GCM before IndexedDB storage (the noisy vector stays in plaintext so it can be searched)
