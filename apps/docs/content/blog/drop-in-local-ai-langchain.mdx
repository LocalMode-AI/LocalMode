...
faq:
  - question: "How do I migrate a LangChain.js app from OpenAI to local models?"
    answer: "Replace three provider constructors: OpenAIEmbeddings becomes LocalModeEmbeddings (wrapping any @localmode/core EmbeddingModel), ChatOpenAI becomes ChatLocalMode (wrapping any LanguageModel), and PineconeStore becomes LocalModeVectorStore (backed by IndexedDB). All downstream chain logic, retrievers, and output parsers stay identical."
  - question: "What LangChain.js features are supported by the LocalMode adapters?"
    answer: "Fully supported: embedDocuments/embedQuery, invoke/generate, real token-by-token streaming, addDocuments/similaritySearch, fromDocuments/fromExistingIndex factories, asRetriever(), cross-encoder reranking via LocalModeReranker, metadata filters, and LangChain callbacks. Not supported: tool calling and structured output via LangChain schemas."
  - question: "Can I use local models for retrieval and cloud models for generation in the same LangChain pipeline?"
    answer: "Yes. Use LocalModeEmbeddings and LocalModeVectorStore for the retrieval pipeline (runs locally at zero cost, keeps documents private) and keep ChatOpenAI for the generation step when frontier model quality is needed. This hybrid approach gives roughly 90% cost savings while maintaining answer quality."
  - question: "What models are recommended for a local LangChain.js RAG pipeline?"
    answer: "BGE-small-en-v1.5 (33 MB, 384 dimensions) for embeddings via @localmode/transformers, and Qwen3 1.7B (~1.1 GB) for chat via @localmode/webllm. The Knowledge Base block at localmode.ai/blocks/knowledge/rag-chat demonstrates this exact combination with document ingestion and source-cited answers."
---
...
`@localmode/langchain` depends on `@langchain/core` as a regular dependency (supported range `>=0.3.80 <1.0.0 || >=1.1.0 <2.0.0`: releases before 0.3.80 carry a serialization-injection flaw, and 1.0.0 through 1.0.6 shipped a broken build), and on `@localmode/core` (>=2.3.0) as a peer dependency. The provider packages (`@localmode/transformers`, `@localmode/webllm`) supply the actual models.
...
<Callout type="info" title="Working demo">
  The [Knowledge Base block](https://localmode.ai/blocks/knowledge/rag-chat) implements this exact pattern with a full UI: paste a document, ask questions, and see retrieved sources with relevance scores. All inference runs in your browser. [View the source code](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/knowledge/rag-chat).
</Callout>
...
- [Knowledge Base block](https://localmode.ai/blocks/knowledge/rag-chat) - the live LangChain RAG demo, with a core ⇄ LangChain engine toggle
