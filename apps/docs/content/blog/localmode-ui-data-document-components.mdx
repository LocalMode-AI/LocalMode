...
It runs every dropped file through the copy-owned `validateFile()` helper (installed with it from `@localmode/ui/lib/browser-utils`) against your `accept` MIME list and a `maxSize`, then emits only the valid files via `onUpload`. Rejected files go to `onReject` with a reason, so you can show the user exactly why a file bounced.
