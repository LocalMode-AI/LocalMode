...
faq:
  - question: "How does LocalMode encrypt embedding vectors in the browser?"
    answer: "LocalMode uses AES-256-GCM encryption via the Web Crypto API. Vectors are encrypted by passing the Float32Array buffer directly to AES-GCM with a fresh 12-byte IV per operation. The IV is prepended to the ciphertext and the result is stored as a padded Float32Array in IndexedDB."
  ...
  - question: "Can you search encrypted vectors with similarity search?"
    answer: "Not directly. Encrypted vectors are ciphertext with no mathematical relationship to the originals, so HNSW similarity search will not return meaningful results. The practical pattern is to decrypt vectors into memory during an active session, search against plaintext in memory, and encrypt only at rest."
...
```typescript
// Keystore is used internally by the Encrypted Vault block.
...
This gives you zero-knowledge storage (nothing readable on disk without the password) while maintaining full-speed HNSW search during active sessions. The [Encrypted Vault block](https://localmode.ai/blocks/privacy/encrypted-vault) demonstrates this pattern.
