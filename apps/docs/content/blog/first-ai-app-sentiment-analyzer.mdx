...
faq:
  - question: "How do I build a sentiment analyzer in JavaScript without Python or a server?"
    answer: "Install two packages (@localmode/core and @localmode/transformers), create a classifier model with transformers.classifier('Xenova/distilbert-base-uncased-finetuned-sst-2-english'), and call classify({ model, text }). The 67 MB model downloads once, caches in the browser, and returns a label (POSITIVE/NEGATIVE) with a confidence score."
  - question: "What model powers the browser-based sentiment analyzer and how accurate is it?"
    answer: "DistilBERT fine-tuned on the Stanford Sentiment Treebank (SST-2) is the default model. It is approximately 67 MB quantized and returns confidence scores between 0 and 1. For clear sentiment like 'I love this product!' it returns POSITIVE with 0.9998 confidence."
  - question: "How long does it take to build a sentiment analyzer with LocalMode?"
    answer: "About 15 minutes from start to finish. The AI portion is five lines of code: import classify and transformers, create the model, and call classify(). The rest is standard React UI (textarea, button, result display). The first analysis takes a few seconds while the model downloads; subsequent analyses are near-instant."
---
...
- **Node.js 20.9+** installed (the minimum for Next.js 16)
...
1. **Model download** -- The `transformers.classifier()` call triggered a one-time download of the DistilBERT ONNX model (~67 MB) from HuggingFace Hub. The browser cached it in the Cache API so future visits skip this step entirely.
...
<Callout type="info" title="See the installable block">
  The [Sentiment Analyzer block](https://localmode.ai/blocks/text-insights/sentiment-analyzer) brings batch analysis, progress tracking, and a polished dashboard UI -- all built with the same `classify()` function you just learned. It runs single and large-batch sentiment with throughput. Install it with `npx shadcn add @localmode/ui/blocks/text-insights/sentiment-analyzer`.
</Callout>
