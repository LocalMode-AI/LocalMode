...
faq:
  - question: "Does LocalMode's LangChain adapter support all LangChain chain types?"
    answer: "The adapters implement standard LangChain interfaces (Embeddings, BaseChatModel, VectorStore) from @langchain/core, so they work with any chain or LCEL expression that uses these interfaces. Use the modern createRetrievalChain API instead of deprecated RetrievalQAChain."
  - question: "Can I import my Pinecone vectors into LocalMode?"
    answer: "Yes. Use @localmode/core's importFrom() to migrate vectors from Pinecone to a local VectorDB. Note that if your Pinecone vectors were created with OpenAI embeddings, you will need to re-embed with a LocalMode model since the vector spaces differ."
  - question: "How many imports do I need to change to switch from cloud to local LangChain?"
    answer: "Three imports: LocalModeEmbeddings replaces OpenAIEmbeddings, ChatLocalMode replaces ChatOpenAI, and LocalModeVectorStore replaces PineconeStore. Existing chains, retrievers, and agents work unchanged."
---
