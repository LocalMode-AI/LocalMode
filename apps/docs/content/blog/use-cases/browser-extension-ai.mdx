...
### Which extension contexts support LocalMode?

All of them. Content scripts, background/service workers, offscreen documents, side panels, popup pages, and options pages. For heavy tasks (LLM inference), use offscreen documents to avoid blocking the extension UI.

### How do I handle model downloads in an extension?

Use `preloadModel()` (from `@localmode/transformers` or your chosen provider package) during extension installation or first use. Models cache in the extension's IndexedDB storage (shared across all extension contexts). Subsequent loads are instant from cache.
