...
With SQ8 quantization, 100K products use ~37MB of IndexedDB storage for 384-dimensional vectors (compared to ~147MB unquantized). Most browsers allow significantly more than that - Chrome grants up to 60% of total disk size per origin. For larger catalogs, use Product Quantization (PQ) for 8-32× compression.

### Does it replace Algolia entirely?

For semantic search, yes. For faceted filtering (price ranges, categories, attributes), you'd still need client-side filtering logic alongside the vector search. LocalMode's VectorDB supports metadata filters, which cover most faceted search needs.
