...
faq:
  - question: "How much storage do AI models need in a PWA?"
    answer: "BGE-small (33MB) for embeddings, DistilBERT (65MB) for classification, and Moonshine-tiny (50MB) for speech form a complete AI toolkit in ~150MB. LLMs add 70MB-5GB depending on model size. Use getStorageQuota() to check available space."
  - question: "Do cached AI models survive PWA app updates?"
    answer: "Yes. Models are cached by the browser, which persists across app updates and browser restarts. The only exception is Safari Private Browsing, which clears IndexedDB on close — use MemoryStorage as a fallback."
  - question: "Can a PWA download AI models during installation?"
    answer: "Yes. Use preloadModel() with an onProgress callback during the PWA install flow. Combine with Service Workers to cache the LocalMode library and app code, so everything works offline after setup."
---
...
- [Serwist - GitHub](https://github.com/serwist/serwist) - service worker library used in the LocalMode UI app
