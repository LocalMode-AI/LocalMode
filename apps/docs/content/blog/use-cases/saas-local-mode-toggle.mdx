...
faq:
  - question: "How should I market a local processing toggle to customers?"
    answer: "Position it as a competitive advantage: 'Your data, your device.' Enterprise buyers actively look for this capability. Some SaaS companies charge a premium for the local processing tier, while others use it as a differentiator in their standard plan."
  - question: "Do users notice quality differences between local and cloud AI?"
    answer: "For embeddings and classification, quality is nearly indistinguishable. For LLM generation, local models (1-4B parameters) are noticeably less capable than GPT-5 on complex tasks but adequate for most chat and summarization use cases."
  - question: "How much code changes are needed to add a local mode toggle?"
    answer: "Minimal. LocalMode's function-first API means embed(), streamText(), and classify() all accept a model parameter that can be either local or cloud. A simple getModel(isLocal) function switches the inference backend without changing application logic."
---
