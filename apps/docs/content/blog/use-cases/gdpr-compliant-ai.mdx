...
faq:
  - question: "Does on-device AI processing avoid GDPR data processor obligations?"
    answer: "For the AI processing itself, yes — if user data never leaves the device, there is no processing by a third-party data processor. However, your app may still have other GDPR obligations like storing user accounts or analytics. You remain the data controller with usual obligations."
  - question: "Does downloading a model involve GDPR data transfer?"
    answer: "No. Model downloads are one-directional: the model comes to the device, no user data goes to the model server. This is similar to downloading a font or JavaScript file and is not considered data processing under GDPR."
  - question: "Is the GDPR guidance on this page legal advice?"
    answer: "No. This guide discusses general compliance considerations for on-device AI processing. The app developer remains a data controller under GDPR. Consult a qualified data protection attorney for guidance specific to your jurisdiction and use case."
---
...
3. **Offline capability** - Once models are cached by the browser, the entire feature works without internet. This is critical for field deployments, mobile apps with spotty connectivity, and enterprise environments with restricted networks.

## Technology Stack

| Package | Purpose |
| --- | --- |
| `@localmode/core` | piiRedactionMiddleware(), wrapEmbeddingModel(), createVectorDB(), wrapVectorDB(), encryptionMiddleware(), deriveEncryptionKey() |
...
// 2. Encrypted metadata at rest (AES-256-GCM, key derived from the user's passphrase).
// Vectors stay in plaintext because the index computes distances over them.
const { key } = await deriveEncryptionKey(userPassphrase, storedSalt);
const db = wrapVectorDB({
  db: await createVectorDB({ name: 'gdpr-safe', dimensions: 384 }),
...
await db.add({ id: '1', vector: embedding, metadata: { text: userText } }); // text is encrypted before IndexedDB
```

The encryption middleware protects the metadata (the text and any other fields) on disk. The stored vector is not encrypted, which is why the PII redaction in step 1 matters: the embedding never encodes an email, phone number, or SSN in the first place. To protect vectors at rest outside the live index, encrypt an exported snapshot with `encryptVector()`.
