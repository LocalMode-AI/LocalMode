...
faq:
  - question: "How accurate is zero-shot moderation with LocalMode?"
    answer: "For clear-cut cases like explicit toxicity and obvious spam, accuracy is typically high — the nli-deberta-v3-xsmall model achieves ~87-92% on standard NLI benchmarks. For subtle cases like sarcasm or coded language, accuracy drops. Zero-shot lets you customize categories without training data."
  - question: "Can LocalMode moderate images in addition to text?"
    answer: "Yes. Combine text classification with image classification (ViT) for multimodal moderation. Classify images for inappropriate content categories and combine scores with text moderation for a holistic decision."
  - question: "Does local content moderation have any per-request costs?"
    answer: "No. After the initial model download, every inference operation is free with zero per-request fees. This makes it ideal for high-volume moderation where cloud APIs would be cost-prohibitive."
---
