...
faq:
  - question: "Does LocalMode semantic search work with Next.js?"
    answer: "Yes. LocalMode models run client-side, so use 'use client' directives on components that call hooks. The model downloads happen in the browser, not during SSR. The LocalMode blocks gallery is itself built with Next.js 16."
...
- LocalMode source - `packages/react/src/index.ts` - full hook export list (59 `use*` hooks confirmed)
