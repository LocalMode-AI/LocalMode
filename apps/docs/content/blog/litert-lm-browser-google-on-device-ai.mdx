...
faq:
  - question: "What is LiteRT-LM and how does it run in the browser?"
    answer: "LiteRT-LM is Google's on-device LLM inference engine, the same technology behind Gemini Nano in Chrome and Pixel devices. It compiles to WebAssembly and runs .litertlm model files on a WebGPU backend with a CPU fallback, accessible in the browser via the @litert-lm/core npm package."
  - question: "What models are available in the @localmode/litert catalog?"
    answer: "The catalog ships three models: Gemma 4 E2B (2.0 GB, WebGPU only), Gemma 4 E4B (3.0 GB, WebGPU only), and Qwen3 0.6B (614 MB, WebGPU or CPU). All three were verified end-to-end in a real Chrome session."
  - question: "Does @localmode/litert support multimodal input like images or audio?"
    answer: "Not yet. While the Gemma 4 .litertlm files include vision and audio encoders, the LiteRT-LM JavaScript API (v0.12.1) does not expose those modalities. The provider is text-only for now. For image input in the browser, @localmode/transformers with Qwen3.5 vision models is the current option."
  - question: "Can I switch between LiteRT-LM and other LocalMode LLM providers?"
    answer: "Yes. All four providers -- WebLLM, Transformers.js, wllama, and LiteRT-LM -- implement the same LanguageModel interface. Swapping providers requires changing only the model import; your streamText() and generateText() calls remain identical."
---
...
<Callout type="info" title="Try it yourself">
  Visit the [/blocks gallery](https://localmode.ai/blocks) to try 11 installable AI blocks running entirely in your browser. The [Chat block](https://localmode.ai/blocks/chat) lets you pick a backend -- including LiteRT-LM -- and run inference with no sign-up, no API keys, and no data leaving your device.
