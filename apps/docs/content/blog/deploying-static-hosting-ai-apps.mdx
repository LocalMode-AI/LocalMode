...
faq:
  - question: "What HTTP headers are required to deploy a LocalMode AI app to static hosting?"
    answer: "Three critical headers: Cross-Origin-Opener-Policy (same-origin) and Cross-Origin-Embedder-Policy (credentialless) enable SharedArrayBuffer for multi-threaded WASM inference, and Content-Security-Policy with wasm-unsafe-eval allows WebAssembly compilation. Without COOP/COEP, wllama falls back to single-threaded mode at 2-4x slower inference."
  - question: "Can I self-host AI model files on Cloudflare Pages?"
    answer: "No. Cloudflare Pages has a 25 MiB per-file limit, and even the smallest embedding model (33 MB) exceeds it. Use Cloudflare R2 for self-hosted models or let models load from HuggingFace CDN by default. Vercel and Netlify have higher limits but the HuggingFace CDN approach is recommended for all platforms."
  - question: "Should I use a static export or server mode for deploying a LocalMode Next.js app?"
    answer: "Static export (output: 'export') is usually the right choice since LocalMode apps have no server-side inference. It works on any static host. Use server mode (output: 'standalone') only if you need SSR for landing pages, metadata generation, or dynamic OG images alongside your client-side AI features."
  - question: "Why use COEP credentialless instead of require-corp for browser AI apps?"
    answer: "The require-corp value blocks all cross-origin resources unless they include a Cross-Origin-Resource-Policy header. Since you do not control HuggingFace CDN response headers, credentialless is the practical choice -- it enables cross-origin isolation while allowing cross-origin model file fetches. Note that credentialless is not supported in Safari as of mid-2026."
---
...
Without this, the browser will block `WebAssembly.instantiate()` and your models will not load. The `wasm-unsafe-eval` directive is [supported in Chrome 97+, Edge 97+, Firefox 102+, and Safari 16+](https://caniuse.com/mdn-http_headers_content-security-policy_script-src_wasm-unsafe-eval).
...
Vercel is the most natural fit for Next.js apps (including the [LocalMode blocks app](https://localmode.ai)). Configuration happens in `next.config.ts` for header rules and `vercel.json` for platform-level overrides.
...
This is not a problem in practice. LocalMode providers (`@localmode/transformers`, `@localmode/webllm`, `@localmode/wllama`) download models from HuggingFace CDN by default. The browser caches them locally in the Cache API or OPFS. Your Cloudflare Pages site serves only the application code.
...
- [ ] **Check file size limits** -- Do not attempt to deploy model files to Cloudflare Pages (25 MiB per-file limit)
...
- [Cloudflare Pages Limits](https://developers.cloudflare.com/pages/platform/limits/) -- 25 MiB per-file limit, 20,000 file free-tier limit
- [Cloudflare Pages File Limit Increase (January 2026)](https://developers.cloudflare.com/changelog/post/2026-01-23-pages-file-limit-increase/) -- Paid plans increased to 100,000 files
- [Vercel Limits](https://vercel.com/docs/limits) -- Deployment source file and function size limits (100 MB Hobby / 1 GB Pro static file uploads; edge function 1 MB)
- [Vercel Function Bundle Sizes](https://vercel.com/kb/guide/troubleshooting-function-250mb-limit) -- 250 MB uncompressed serverless function limit
- [Netlify Custom Headers](https://docs.netlify.com/manage/routing/headers/) -- `_headers` file and `netlify.toml` syntax
- [Netlify Pricing](https://www.netlify.com/pricing/) -- 100 GB/month bandwidth on free tier
...
- [Can I Use: COEP credentialless](https://caniuse.com/mdn-http_headers_cross-origin-embedder-policy_credentialless) -- Browser support: Chrome 96+, Edge 96+, Firefox 119+; Safari not supported
- [Can I Use: wasm-unsafe-eval CSP](https://caniuse.com/mdn-http_headers_content-security-policy_script-src_wasm-unsafe-eval) -- Chrome 97+, Edge 97+, Firefox 102+, Safari 16+
