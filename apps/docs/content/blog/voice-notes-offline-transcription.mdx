...
faq:
  - question: "Can speech-to-text run entirely in the browser without a server?"
    answer: "Yes. Moonshine is a speech recognition model designed for edge devices that runs in the browser via ONNX Runtime and Transformers.js. The model downloads once (27-237MB depending on variant (Moonshine Tiny ~27-50MB, Moonshine Base ~237MB)), caches itself, and works offline from that point forward."
  - question: "How does Moonshine compare to Whisper for voice transcription?"
    answer: "Moonshine Tiny (27M params) achieves 4.52% word error rate on LibriSpeech clean, beating Whisper tiny.en's 5.66% with 28% fewer parameters. Moonshine Base (61.5M params) achieves 3.23% WER, close to the Whisper API's large-v3 at ~2.7%. Moonshine also uses 5x less compute than Whisper for a 10-second segment."
  - question: "How much does browser-based speech-to-text cost compared to cloud APIs?"
    answer: "Zero after the initial model download. OpenAI's Whisper API charges $0.006 per minute. For a voice notes app with 10,000 daily active users averaging 5 minutes of audio each, that totals $109,500 per year in cloud costs versus $0 with Moonshine running locally."
  - question: "Why is Moonshine more efficient than Whisper in the browser?"
    answer: "Moonshine uses Rotary Position Embeddings and processes audio proportional to its actual duration, unlike Whisper which pads all audio to fixed 30-second chunks. A 3-second voice note uses a fraction of the compute that a 30-second chunk would require."
---
...
For non-English audio, pass a language hint or use `task: 'translate'` to translate to English. Moonshine is English-only, so this requires a multilingual Whisper model:
...
The [Voice Notes block](https://localmode.ai/blocks/audio/voice-notes) extends this pattern with accumulated notes. It uses `useOperationList` from `@localmode/react` to maintain a growing list of transcribed notes, each with its audio URL and timestamp:
...
The [Meeting Assistant block](https://localmode.ai/blocks/audio/meeting-assistant) takes this further - it transcribes uploaded audio files with Moonshine Base (`onnx-community/moonshine-base-ONNX`, ~237MB) for higher accuracy, then pipes the transcript through a summarization model to extract action items and key decisions.

---

## Offline and PWA: Works on a Plane

Once the Moonshine model downloads, it is cached in the browser via the Transformers.js cache (backed by the Cache API). From that point on, transcription works with zero network access.
...
- **Your users are on low-end devices** - Transcription uses the device's own CPU/GPU, and live streaming means inference runs concurrently with recording. Very old phones or tablets may struggle; cloud APIs offload that compute entirely.

One reason that used to be on this list no longer belongs there: **real-time streaming**. `createLiveTranscriber()` from `@localmode/core` streams microphone audio through the same on-device models as the user speaks - a voice activity detector (energy-based or Silero) segments the stream into utterances, partial text is emitted at a configurable cadence while speech is still in progress, and each utterance is finalized the moment it ends. Long recordings are no problem either: audio is transcribed utterance by utterance instead of as one post-recording blob. See the [Live Transcribe guide](/docs/core/live-transcribe) or try the [Live Transcription block](https://localmode.ai/blocks/audio/live-transcription).
...
- **Code examples**: All API signatures verified against LocalMode source code - `packages/core/src/audio/transcribe.ts`, `packages/core/src/audio/live-transcribe.ts`, `packages/react/src/hooks/use-transcribe.ts`, and `packages/react/src/utilities/use-voice-recorder.ts`.
