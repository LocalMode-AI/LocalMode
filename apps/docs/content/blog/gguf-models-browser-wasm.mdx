...
| Available models | 160K+ GGUF models | ~32 curated MLC models |
...
While you can load any of the 160,000+ GGUF models on HuggingFace, `@localmode/wllama` ships a curated catalog of 18 models that have been tested and verified to work well with the wllama runtime (llama.cpp b7179). All are Q4_K_M quantized for the best size-to-quality ratio in browser environments.
...
1. **Browse** -- View the curated catalog of 18 models grouped by size tier (tiny, small, medium, large), or paste any HuggingFace GGUF URL
...
- All code examples reference the `@localmode/wllama` package (wrapping `@wllama/wllama` ^2.2.1 and `@huggingface/gguf` ^0.1.14) and the GGUF Explorer showcase app in the LocalMode repository
