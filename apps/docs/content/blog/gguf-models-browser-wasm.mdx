---
title: "GGUF Models in the Browser: 160,000+ Models Via llama.cpp WASM"
description: "Run any of 160,000+ GGUF models from HuggingFace directly in the browser using llama.cpp compiled to WebAssembly. No WebGPU required. Inspect model metadata before downloading, check device compatibility, and stream text generation -- all with the same LanguageModel interface you already know."
author: LocalMode
date: 2026-03-17
dateModified: "2026-07-04"
faq:
  - question: "What is GGUF and how many GGUF models are available for browser inference?"
    answer: "GGUF (GGML Universal File) is a self-contained format for quantized LLMs created by the developer of llama.cpp. A single GGUF file bundles model weights, architecture metadata, and tokenizer configuration. As of May 2026, HuggingFace hosts over 178,000 GGUF models, all runnable in the browser via @localmode/wllama."
  - question: "Why does @localmode/wllama use WebAssembly instead of WebGPU?"
    answer: "WebAssembly has been supported in every major browser since 2017 (Chrome 57+, Firefox 52+, Safari 11+), while WebGPU is still not universally available (82% coverage in mid-2026). WASM runs at roughly 40-50% of WebGPU speed on CPU but works everywhere. Optional WebGPU acceleration is available when the browser supports it."
  - question: "Can I inspect a GGUF model's metadata without downloading the full file?"
    answer: "Yes. The parseGGUFMetadata() function reads just the file header (roughly 4 KB) via an HTTP Range request. It returns architecture, context length, quantization type, parameter count, vocabulary size, layer count, and model name -- letting you check device compatibility before committing to a multi-gigabyte download."
  - question: "How does @localmode/wllama compare to @localmode/webllm?"
    answer: "Wllama supports 30 curated plus 160,000+ GGUF models with universal browser support, 3 embedding models, tool calling, and vision via mmprojUrl. WebLLM offers 32 curated MLC-compiled models requiring WebGPU with native GPU speed. Wllama trades some speed for vastly broader model selection and compatibility."
---
...
This is the same pattern used in the [Device & Model Lab block](https://localmode.ai/blocks/device/gguf-explorer), where users browse the curated catalog, inspect any model, and see compatibility results before handing off to the chat block to download and run it.
...
## The Device & Model Lab: Try It Live

The [Device & Model Lab block](https://localmode.ai/blocks/device/gguf-explorer) puts all of this together in one flow:

1. **Browse** -- View the curated catalog of 30 models (25 language + 3 embedding + 2 reranker) grouped by size tier, or paste any HuggingFace GGUF URL
...
3. **Chat** -- Hand off to the [Chat block](https://localmode.ai/blocks/chat) to download the model and start a streaming conversation

The block demonstrates the `@localmode/wllama` inspection surface: `WLLAMA_MODELS` for the catalog, `parseGGUFMetadata()` for inspection, and `checkGGUFBrowserCompat()` for compatibility -- no model download required. It hands the chosen model to the chat block for `wllama.languageModel()` + `streamText()` inference. Every operation runs entirely in the browser.
...
- [GGUF - Wikipedia](https://en.wikipedia.org/wiki/GGUF) -- Introduction date (21 August 2023) and format history confirmed here
- [wllama](https://github.com/ngxson/wllama) -- WebAssembly binding for llama.cpp by ngxson, enabling browser-based inference; releases verified at github.com/ngxson/wllama/releases
- [HuggingFace GGUF model library](https://huggingface.co/models?library=gguf) -- Live GGUF model count: 178,541 as of May 2026
...
- [WebGPU browser support - caniuse.com](https://caniuse.com/webgpu) -- Chrome 113+, Edge 113+, Safari 26+ (partial); Firefox Windows 141+, macOS ARM64 145+
- [WebAssembly browser support - caniuse.com](https://caniuse.com/wasm) -- Supported since Chrome 57, Firefox 52, Safari 11, Edge 16 (all 2017)
- [WebGPU vs WebAssembly benchmarks](https://www.sitepoint.com/quantization-q4km-vs-awq-fp16-local-llms/) -- Performance comparisons between WASM and WebGPU for browser-based ML inference
- All code examples reference the `@localmode/wllama` package (wrapping `@wllama/wllama` ^3.2.3 and `@huggingface/gguf` ^0.1.14) and the GGUF Explorer block in the LocalMode repository
