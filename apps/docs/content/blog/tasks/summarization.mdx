...
faq:
  ...
  - question: "What browsers support text summarization with LocalMode?"
    answer: "Transformers.js models work in all modern browsers via WebGPU or WASM backends. Chrome Built-in AI's Summarizer API is stable in Chrome 138+ on desktop, with no flag required; Chrome downloads the on-device model once, browser-wide, on first use. LocalMode provides a fallback pattern to try Chrome AI first and fall back to DistilBART."
