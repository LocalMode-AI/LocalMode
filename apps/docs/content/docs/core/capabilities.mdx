...
<Callout type="info" title="See it in action">
  Try the [Chat block](https://localmode.ai/blocks/chat) and [Knowledge Base block](https://localmode.ai/blocks/knowledge) for working demos of these APIs.
</Callout>
...
| `isOPFSSupported()` | `boolean` | Origin Private File System (`navigator.storage.getDirectory()` is available) |
...
Compute an optimal batch size for `streamEmbedMany()`, `streamEmbedManyImages()`, or `ingest()` based on device hardware:
...
<Callout type="info" title="See it live">
  The [Device & Model Lab block](https://localmode.ai/blocks/device) runs `detectCapabilities()` and `recommendModels()` in your browser — a live device capability report with ranked model recommendations, no model download required.
</Callout>
...
|-----|-------------|-------|
| Chat (LLM Chat) | WebGPU detection for optimal model backend selection | [Live block](https://localmode.ai/blocks/chat) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/chat) |
| Knowledge Base (PDF Search) | Feature detection for WebGPU-accelerated search | [Live block](https://localmode.ai/blocks/knowledge/semantic-search) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/knowledge/semantic-search) |
