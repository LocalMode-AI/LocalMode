...
<Callout type="info" title="See it in action">
  Try the [Knowledge Base block](https://localmode.ai/blocks/knowledge/vector-data-manager) for a working demo of these APIs.
</Callout>
...
<Callout type="info" title="External storage adapters must persist the fingerprint">
The `modelFingerprint` lives on the `Collection` object. Drift detection only works across a reopen if the storage adapter round-trips it. The official adapters (`@localmode/dexie`, `@localmode/idb`, `@localmode/localforage`) now persist the full `Collection` (see the [conformance suite](/docs/core/storage#verify-with-the-conformance-suite)); collections written by an earlier adapter version stored no fingerprint and silently skip drift detection until re-ingested.
</Callout>
...
|-----|-------------|-------|
| Knowledge Base (Semantic Search) | Detect model drift and trigger reindexing | [Live block](https://localmode.ai/blocks/knowledge/vector-data-manager) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/knowledge/vector-data-manager) |
