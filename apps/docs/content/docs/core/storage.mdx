...
<TypeTable
  type={{
    ...
    'getVector(id)': {
      description: 'Get a vector by ID (Uint8Array when SQ8/PQ-compressed)',
      type: 'Promise<Float32Array | Uint8Array | null>',
    },
    'deleteVector(id)': {
    ...
    'getAllVectors(collectionId)': {
      description: 'Get all vectors in a collection (Uint8Array values when SQ8/PQ-compressed)',
      type: 'Promise<Map<string, Float32Array | Uint8Array>>',
    },
...
class MyCustomStorage implements StorageAdapter {
  ...
  async getVector(id: string) { /* Float32Array (or Uint8Array if you store compressed) or null */ return null; }
...
<Callout type="warn" title="Persist the full Collection object">
Collection write and read operations must round-trip **every** field of the
`Collection` object — including the optional extended fields core stores on it
(`modelFingerprint` for drift detection, `calibration`/`pqCodebook` for vector
quantization, `compressionCalibration`/`deltaCalibration`/`compression` for
storage compression). Persist `{ ...collection }` rather than cherry-picking
known fields: an adapter that drops these silently corrupts SQ8/PQ-compressed
vectors after a reopen and disables drift detection.
</Callout>

### Verify with the conformance suite

Core's testing utilities ship a framework-agnostic StorageAdapter conformance
suite. It covers the full contract — document/vector/index/collection
operations, `Uint8Array` payload fidelity, extended-field round-trips, and
close→reopen persistence (including an SQ8 compression fidelity check). Run it
against any custom adapter:
...
describe('MyCustomStorage conformance', () => {
  const suite = createStorageAdapterConformanceSuite(async () => {
    const name = `conformance-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const adapter = new MyCustomStorage({ name });
    await adapter.open();
    return {
      adapter,
      // A NEW adapter instance over the SAME underlying database
      reopen: async () => {
        const reopened = new MyCustomStorage({ name });
        await reopened.open();
        return reopened;
      },
    };
...
The official adapters (`@localmode/dexie`, `@localmode/idb`,
`@localmode/localforage`) run this same suite in their own test suites.
...
|-----|-------------|-------|
| Privacy (Encrypted Vault) | Encrypted client-side storage with Web Crypto API | [Live block](https://localmode.ai/blocks/privacy/encrypted-vault) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/privacy/encrypted-vault) |
