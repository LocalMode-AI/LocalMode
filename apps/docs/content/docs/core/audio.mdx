...
<Callout type="info" title="See it in action">
  Try the [Audio Studio block](https://localmode.ai/blocks/audio) for a working demo of these APIs.
</Callout>

## transcribe()

<Callout type="info" title="Real-time transcription">
  For streaming microphone-driven speech-to-text with VAD, push-to-talk, open-mic, and conversational voice loops, see [`createLiveTranscriber()` and `createTurnTaker()`](/docs/core/live-transcribe).
</Callout>
...
Specify a language hint or translate non-English audio to English. These options require a **multilingual** model (e.g. `Xenova/whisper-tiny`) — English-only checkpoints such as Moonshine or `whisper-tiny.en` reject `language`/`task` hints other than `'en'`/`'transcribe'`:

```ts
const whisper = transformers.speechToText('Xenova/whisper-tiny');
...
<Callout type="info" title="Streaming synthesis">
  For real-time voice loops where users need to hear the first words while later clauses are still synthesizing, use [`streamSynthesizeSpeech()` and `playStreamedSpeech()`](/docs/core/streaming-speech) instead. They yield clause-by-clause `Float32Array` audio and play it gap-free on a Web Audio queue.
</Callout>
...
<TypeTable
  type={{
    ...
    voice: {
      description: "Voice ID to use for synthesis — Kokoro provides 29 English voices (e.g. 'af_heart', 'bf_emma', 'am_michael')",
      type: 'string',
      required: false,
    },
    speed: {
...
<Cards>
  ...
  <Card
    ...
    description="Stream TTS clause-by-clause for low first-audio latency."
  />
  <Card
    title="Live Transcribe"
    href="/docs/core/live-transcribe"
    description="Streaming microphone STT with VAD, push-to-talk, and voice loops."
  />
  <Card
    title="Middleware"
...
|-----|-------------|-------|
| Audio Studio (Voice Notes) | Record and transcribe audio with Moonshine models | [Live block](https://localmode.ai/blocks/audio) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/audio) |
| Audio Studio (Meeting Assistant) | Transcribe and summarize meeting recordings | [Live block](https://localmode.ai/blocks/audio) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/audio) |
| Audio Studio (Audiobook Creator) | Generate speech audio from text | [Live block](https://localmode.ai/blocks/audio) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/audio) |
