...
<Callout type="info" title="See it in action">
  Try the [Knowledge Base block](https://localmode.ai/blocks/knowledge/rag-chat) for a working demo of these APIs.
</Callout>
...
// Each result carries the chunk text stored by ingest()
// on results[].text — see "Text round-trip" below.
```
...
`ingest()` chunks, embeds, and stores documents in a single call. It is callable in **two equivalent forms**:

- **Object form** — `ingest({ db, documents, model?, ...options })`. Pass an `EmbeddingModel` as `model` and embeddings are generated internally via `embedMany()`; `generateEmbeddings` defaults to `true` when `model` is set.
- **Positional form** — `ingest(db, documents, options?)`, typically with `generateEmbeddings: true` and a custom `embedder` callback (see [Batch Processing](#batch-processing--ingestion-pipeline) below).
...
<Callout type="warn">
  `model` and `embedder` are mutually exclusive ways to generate embeddings — pass exactly one. Providing both throws. To chunk and index without embedding, pass an explicit `generateEmbeddings: false` (it wins over the `model` default).
</Callout>

### Text Round-Trip with semanticSearch

`ingest()` stores each chunk's text on the document's metadata under the exported constant `TEXT_METADATA_FIELD` (`'_text'`). `semanticSearch()` resolves that key onto `results[].text`, so an ingest → search round-trip returns the chunk text out of the box:

```typescript
import { ingest, semanticSearch, TEXT_METADATA_FIELD } from '@localmode/core';

await ingest({
  db,
  model,
  documents: [{ text: 'The reactor codename is Firefly.' }],
});

const { results } = await semanticSearch({ db, model, query: 'reactor codename', k: 3 });
console.log(results[0].text); // 'The reactor codename is Firefly.'
console.log(results[0].metadata?.[TEXT_METADATA_FIELD]); // same chunk text
```
...
await ingest({
  ...
  onProgress: (progress) => {
    console.log(`${progress.phase}: ${progress.chunksProcessed}/${progress.totalChunks} chunks`);
  },
});
```

The `onProgress` callback reports four phases — `chunking`, `embedding`, `indexing`, `complete` — with `documentsProcessed`/`totalDocuments`, `chunksProcessed`/`totalChunks`, and `currentBatch`/`totalBatches` counters.

### Cancellation

Both call forms accept an `abortSignal`, checked before chunking and between each embedding/indexing batch:
...
<Callout type="info">
  Documents already added to the database before an abort are **not** rolled back — the promise rejects with the abort reason and no further batches are embedded or indexed.
</Callout>
...
Each result's `text` is extracted from its metadata (checking `text`, then `content`, `body`, `_text` — the `TEXT_METADATA_FIELD` key written by `ingest()` — `__text`, and `pageContent`), so ingested chunks round-trip their text automatically. For the full API reference, options, and extraction details, see [semanticSearch()](/docs/core/embeddings#semanticsearch).
...
// Semantic search
const { results: semanticResults } = await semanticSearch({ db, model, query, k: 20 });
...
Load documents from various formats. Each loader takes default options in its constructor (options passed to `load()` win), and `load()` returns `LoadedDocument[]` (`{ id, text, metadata }`):
...
const textDocs = await textLoader.load(textBlob);

// JSON
const jsonLoader = new JSONLoader({ textFields: ['content'] });
const jsonDocs = await jsonLoader.load(jsonBlob);

// CSV
const csvLoader = new CSVLoader({ textColumn: 'description' });
const csvDocs = await csvLoader.load(csvBlob);

// HTML
const htmlLoader = new HTMLLoader({ selector: 'article' });
const htmlDocs = await htmlLoader.load(htmlBlob);

// PDF
const pdfLoader = new PDFLoader({ splitByPage: true });
const pdfDocs = await pdfLoader.load(pdfBlob);
```

See [Document Loaders](/docs/core/document-loaders) for every option and for `loadDocument()` auto-detection.
...
`streamSemanticSearch()` accepts the same options as `semanticSearch()` but returns an `AsyncGenerator` that yields results one at a time. Each yielded item has the same shape (and the same `text` extraction) as `semanticSearch()` results.
...
Full ingestion pipeline — chunk, embed, and store in one call. Here in the **positional form** (`ingest(db, documents, options?)`) with a custom `embedder` callback, equivalent to the object form shown in [Ingestion](#ingestion):
...
|-----|-------------|-------|
| Knowledge Base (Semantic Search) | Recursive and semantic chunking for document search | [Live block](https://localmode.ai/blocks/knowledge/semantic-search) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/knowledge/semantic-search) |
| Knowledge Base (PDF Search) | Semantic chunking of PDF content for Q&A | [Live block](https://localmode.ai/blocks/knowledge/semantic-search) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/knowledge/semantic-search) |
| Knowledge Base (LangChain RAG) | End-to-end RAG with chunking and retrieval | [Live block](https://localmode.ai/blocks/knowledge/rag-chat) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/knowledge/rag-chat) |
