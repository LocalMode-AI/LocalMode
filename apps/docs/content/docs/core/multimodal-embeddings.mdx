...
<Callout type="info" title="See it in action">
  Try the [Photo Search block](https://localmode.ai/blocks/photo/image-search) for a working demo of these APIs — its Search tab runs text-to-image and image-to-image queries over one shared CLIP vector space, entirely in the browser.
</Callout>
...
Stream image embeddings as they're generated, with per-batch progress — the multimodal counterpart of [`streamEmbedMany()`](/docs/core/embeddings#streamembedmany):
...
<TypeTable
  type={{
    ...
    adaptiveBatching: {
      description:
        'Compute the batch size from device capabilities when no explicit batchSize is set',
      type: 'boolean',
      default: 'false',
    },
    abortSignal: {
      description: 'AbortSignal for cancellation (checked before every batch)',
      type: 'AbortSignal',
    },
    maxRetries: {
      description: 'Maximum retry attempts per batch',
    ...
    onBatch: {
      description: 'Callback after each batch with { index, count, total, usage }',
      type: '(progress: EmbedProgress) => void',
    },
  }}
/>

Each yielded item is a `StreamEmbedImageResult`: `{ embedding: Float32Array, index: number }`, where `index` is the position of the source image in the input array.
...
|-----|-------------|-------|
| Photo Search (Cross-Modal Search) | Text-to-image and image-to-image search with CLIP | [Live block](https://localmode.ai/blocks/photo/image-search) · [Source](https://github.com/LocalMode-AI/LocalMode/tree/main/apps/ui/src/app/blocks/photo/image-search) |
