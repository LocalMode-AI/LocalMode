...
<Callout type="warn" title="External storage adapters must persist the calibration">
The SQ8/PQ calibration (`calibration`, `pqCodebook`) lives on the `Collection` object. A storage adapter that drops those fields decodes quantized vectors as raw bytes after a reopen. The official adapters (`@localmode/dexie`, `@localmode/idb`, `@localmode/localforage`) now round-trip the full `Collection`, so calibration survives across sessions — but databases quantized with an **earlier** adapter version never stored it and must be cleared and re-ingested. See the [full-`Collection` persistence rule](/docs/core/storage#verify-with-the-conformance-suite) and your adapter's recovery note ([Dexie](/docs/dexie), [idb](/docs/idb), [localForage](/docs/localforage)).
</Callout>
...
const { centroids, assignments, iterations } = kMeansCluster(data, 8, {
  maxIterations: 20,
  threshold: 1e-6,
  // Optional: inject a seeded random source for deterministic clustering
  // (k-means random init can converge to different local optima run-to-run)
