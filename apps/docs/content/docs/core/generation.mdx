...
<Tabs items={['WebLLM (WebGPU)', 'wllama (WASM)', 'Transformers (ONNX)', 'LiteRT (Google)']}>
...
|---|---|---|---|---|
| **Runtime** | MLC WebGPU | llama.cpp WASM + optional WebGPU | Transformers.js v4 ONNX | LiteRT-LM (Google) |
| **GPU Required** | Yes (WebGPU) | No (optional WebGPU acceleration) | No (auto-fallback to WASM) | WebGPU for Gemma 4; Qwen3 0.6B runs on CPU WASM too |
...
| **Model Catalog** | 32 curated MLC models | 30 curated (25 language + 3 embedding + 2 reranker) + 160K+ GGUF | 16 curated ONNX models | 3 verified models |
...
| **Multimodal** | Phi 3.5 Vision | Holo2 4B/8B, Gemma 4 E2B/E4B (vision via mmprojUrl) | 5 vision variants (Qwen3.5 + Gemma 4) | Text-only |
| **Best For** | Maximum speed on GPU-capable devices | Universal compatibility, embeddings, huge model selection | Broad ONNX ecosystem, WebGPU + WASM flexibility | Google's officially-supported on-device pipeline (Gemma 4) |
...
<Callout type="info">
For model catalogs, provider-specific configuration, and detailed setup, see the [WebLLM](/docs/webllm), [wllama](/docs/wllama), [Transformers Text Generation](/docs/transformers/text-generation), and [LiteRT](/docs/litert) guides.
</Callout>
