...
<Callout type="info" title="See it in action">
  Try the [Knowledge Base block](https://localmode.ai/blocks/knowledge) for a fully migrated RAG pipeline — its engine toggle runs the same ingest, search, and grounded answers through the LocalMode adapters and the core pipeline behind identical UI.
</Callout>
