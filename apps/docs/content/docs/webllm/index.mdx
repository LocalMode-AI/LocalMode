...
<Callout type="info" title="See it in action">
  Try the [Chat block](https://localmode.ai/blocks/chat) — a flagship multi-provider chat where WebLLM is one of four selectable backends, with vision attachments, reasoning display, semantic cache, and agent mode.
</Callout>
...
| Research Agent | Tool-using ReAct agent over a WebGPU WebLLM model with a step timeline and a human-in-the-loop tool-approval gate | [Live](https://localmode.ai/blocks/agents/research-agent) · Install: `npx shadcn add @localmode/ui/blocks/agents/research-agent` |
