...
|-|--------|--------|--------------------|--------|
| **Runtime** | WebGPU | WASM (llama.cpp) | ONNX Runtime | Google LiteRT-LM (WebGPU + WASM) |
| **Models** | 32 curated (MLC) | 18 curated + 160K+ GGUF from HuggingFace | 16 ONNX (TJS v4) | 3 curated `.litertlm` — Gemma 4 E2B/E4B, Qwen3 0.6B |
| **Speed** | Fastest (GPU) | Good (CPU) | Good (CPU/GPU) | Good (early preview) |
| **Vision** | Phi 3.5 Vision | Holo2 4B/8B | 5 vision variants (Qwen3.5 + Gemma 4) | — (text-only) |
...
| **Best For** | Maximum performance | Universal compatibility, model variety | Multi-task (embeddings + LLM in one package) | First-party Google runtime for Gemma |
