...
<Callout type="info" title="Browser support">
  The Prompt API shipped for web pages in **Chrome 148+** desktop (Chrome 138 shipped it for *Chrome Extensions* only). Firefox and Safari do not implement it; non-Chromium browsers report `false` from `isPromptAPISupported()` and the model rejects with a typed `GenerationError`.
</Callout>

<Callout type="warn" title="Sampling parameters are still origin-trial gated">
  The Prompt API itself is stable on web pages, but `temperature` and `topK` are not: on a web page they apply only under the "Prompt API sampling parameters" origin trial (they are unrestricted in Chrome Extensions). Treat them as a progressive enhancement — set them if you have the trial, and do not depend on them changing output for ordinary visitors.
</Callout>
...
|---------|------------|-------|
| Chrome 148+ desktop | Stable (web pages) | Top-level `window.LanguageModel` |
...
| `temperature` | `number` | Chrome default | Sampling temperature (0–1). See the sampling-parameters caveat below. |
| `topK` | `number` | Chrome default | Top-K sampling cutoff. See the sampling-parameters caveat below. |
...
<Callout type="info">
  For full API reference, options, and result types, see the [Core Text Generation](/docs/core/generation) guide.
</Callout>
...
|------|------|--------------------|
| `chrome-ai-not-supported` | `window.LanguageModel` is missing entirely | Update to Chrome 148+ stable on desktop; verify the page is not in Incognito mode |
