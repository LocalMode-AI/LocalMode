...
<Callout type="info">
  All three stable APIs (Summarizer, Translator, Prompt) are supported by `@localmode/chrome-ai`. The Prompt API exposes Gemini Nano text generation via `chromeAI.languageModel()` — see the [Language Model](/docs/chrome-ai/language-model) guide.
</Callout>
