...
<Callout type="info" title="See it in action">
  Try the [Knowledge Base block](https://localmode.ai/blocks/knowledge/semantic-search) — it embeds your
  documents into an in-browser VectorDB for semantic search and grounded Q&A.
</Callout>
...
| --- | --- | --- | --- |
| `'webgpu'` | Fastest (GPU-accelerated) | Chrome 113+, Edge 113+, Safari 26+ | Default when available |
...
`@localmode/transformers` caches model files, not embeddings. To avoid recomputing embeddings for repeated inputs, wrap the model with a small `wrapEmbed` cache middleware; the pattern is shown in [Core Embeddings: Middleware](/docs/core/embeddings#middleware).
...
<Callout type="tip" title="Embedding Tips">
  ...
  3. **Cache embeddings** — Wrap the model with a `wrapEmbed` cache middleware for repeated queries
