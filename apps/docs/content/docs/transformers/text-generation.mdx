...
<Callout type="info" title="See it in action">
  Try the [Chat block](https://localmode.ai/blocks/chat) — a multi-provider chat where the
  transformers ONNX models run alongside the WebLLM, wllama, and LiteRT backends, with vision
  input on Qwen3.5 and Gemma 4.
</Callout>

## 5-Tier Fallback

LocalMode offers five LLM providers, each with different trade-offs:

| Provider | Format | Best For | Notes |
|----------|--------|----------|-------|
| `@localmode/chrome-ai` | none (Chrome-managed) | Shipping no model files | Chrome 148+ desktop only (Prompt API shipped for web pages in 148); Gemini Nano; text-only; gate with `isPromptAPISupported()`; Chrome may download the model once, browser-wide |
...
Use a try/catch chain for automatic failover. Chrome AI sits above the chain rather than inside it: `chromeAI.languageModel()` constructs lazily and never throws when the Prompt API is absent, so it must be gated with the synchronous `isPromptAPISupported()` check instead of a `catch`.
...
function downloadableModel() {
  try {
    ...
    return litert.languageModel('qwen3-0.6B');
  } catch {
    try {
      return webllm.languageModel('Llama-3.2-1B-Instruct-q4f16_1-MLC');
    } catch {
      try {
        return transformers.languageModel('onnx-community/Qwen3.5-0.8B-ONNX');
      } catch {
        return wllama.languageModel('Llama-3.2-1B-Instruct-Q4_K_M');
      }
    }
  }
}

// Tier 0: Chrome supplies Gemini Nano — your app fetches no model files.
// Tiers 1-4: fall through the app-managed downloadable providers.
...
The [Chat block](https://localmode.ai/blocks/chat) demonstrates these ONNX models alongside the WebLLM, wllama, and LiteRT backends. Select a Qwen3.5 or Gemma 4 model to try vision input with image uploads.
