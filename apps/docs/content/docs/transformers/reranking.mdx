...
<Callout type="info" title="See it in action">
  Try the [Knowledge Base block](https://localmode.ai/blocks/knowledge/semantic-search) — its Search tab has a toggleable rerank stage with selectable cross-encoders, including the transformers ONNX models below.
</Callout>
...
async function searchWithReranking(query: string) {
  ...
  const { results: candidates } = await semanticSearch({
    db,
...
<Cards>
  ...
  <Card
    title="React Hook"
    href="/docs/react/classification#usererank"
    description="useRerank — rerank documents with built-in loading, error, and cancellation state."
  />
  <Card
    title="RAG"
    href="/docs/core/rag"
