...
<Callout type="info" title="See it in action">
  Try the [Chat block](https://localmode.ai/blocks/chat) — a flagship multi-provider chat where you can run the wllama catalog or load any GGUF by URL, with vision attachments, reasoning display, semantic cache, and agent mode.
</Callout>
...
- **True Streaming** -- Token-by-token streaming via `createChatCompletion({ stream: true })`, including for a bare `prompt`
...
| --- | --- | --- | --- | --- | --- |
| `SmolLM2-135M-Instruct-Q4_K_M` | SmolLM2 135M | 105MB | 8K | 135M | Instant loading, testing |
...
| Catalog Key | Name | Size | Context | Params | Best For |
| --- | --- | --- | --- | --- | --- |
| `Qwen3-0.6B-Q4_K_M` | Qwen3 0.6B | 397MB | 40K | 600M | Fast multilingual reasoning, hybrid thinking |
| `TinyLlama-1.1B-Chat-Q4_K_M` | TinyLlama 1.1B Chat | 670MB | 2K | 1.1B | Classic, fast and reliable |
| `Llama-3.2-1B-Instruct-Q4_K_M` | Llama 3.2 1B | 808MB | 128K | 1.2B | General purpose, huge context |
...
| `bge-small-en-v1.5-Q8_0` | BGE Small EN v1.5 | 37MB | 384 | Lightweight on-device embeddings |
...
`doStream()` streams token-by-token via `createChatCompletion({ stream: true })`, delivering each token as it is generated rather than buffering the full response. A bare `prompt` is a single user turn through the GGUF's chat template, exactly as the WebLLM, Transformers.js, and LiteRT providers treat it, so instruct models see the formatting they were trained on and stream in real time:
...
Raw untemplated completion is still available with `providerOptions: { wllama: { raw: true } }`, and is the automatic path for base GGUFs that ship no chat template. Raw completion has no streaming surface, so the whole generation arrives as a single chunk:

```typescript
const result = await streamText({
  model: wllama.languageModel('my-repo/my-base-model-GGUF:model-Q4_K_M.gguf'),
...
```typescript
const model = wllama.languageModel('Llama-3.2-1B-Instruct-Q4_K_M', {
  useJinja: false, // disable Jinja template parsing (llama.cpp's built-in template matching is used instead)
});
```

To bypass the chat template entirely for a request, pass `providerOptions: { wllama: { raw: true } }` (see [True Streaming](#true-streaming)).
...
### Prompt and Template Options

`providerOptions.wllama` also controls how the request reaches the chat template:
...
- **`raw`** (`boolean`) -- Send the `prompt` as raw untemplated completion via `createCompletion` instead of a user turn through the chat template. Automatic for base GGUFs with no chat template. Raw completion does not stream; the full generation arrives as one chunk.
- **`cache_prompt`** (`boolean`) -- Forwarded to `createChatCompletion`. llama.cpp reuses the prompt KV cache across requests by default, so a repeated prompt skips prefill; set `false` to make every request pay prefill (for example when timing inference).
- **`chat_template_kwargs`** (`object`) -- Forwarded to `createChatCompletion` and made available to the model's Jinja chat template (for example `enable_thinking` on Qwen3 templates).
...
Use `wllama.reranker()` with cross-encoder models to rerank search results by relevance. The catalog ships two reranker models (Jina Reranker v2 and BGE Reranker v2 M3); both are selectable live in the [Knowledge Base block](https://localmode.ai/blocks/knowledge/semantic-search)'s reranked search:
...
| Chat | Multi-provider LLM chat (transformers, WebLLM, wllama, LiteRT) with vision, reasoning, semantic cache, and agent mode | [Live](https://localmode.ai/blocks/chat) · Install: `npx shadcn add @localmode/ui/blocks/chat` |
