...
`@localmode/ai-sdk` implements the AI SDK 6 provider interfaces (`LanguageModelV3`, `EmbeddingModelV3`): it needs `ai` 6.0.0 or later, alongside `@ai-sdk/provider` and `@ai-sdk/provider-utils` as peer dependencies, and is tested against `ai` 6.0.287.
...
- **WebGPU required for LLMs** — WebLLM requires WebGPU (Chrome 113+, Edge 113+, Safari 26+).
