...
function getRelatedUseCases(currentSlug: string, limit = 3) {
  const allPages = useCases.getPages().sort((a, b) => {
...
export default async function UseCasePage(props: {
  ...
  const related = getRelatedUseCases(params.slug);
  ...
  const jsonLd: Record<string, unknown>[] = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: 'Use Cases', item: `${baseUrl}/blog/use-cases` },
    ...
    <article className="flex-1 w-full max-w-[900px] mx-auto px-6 py-16">
      ...
      <nav aria-label="Breadcrumb" className="text-sm text-fd-muted-foreground mb-8">
        <ol className="flex items-center gap-1.5">
          ...
          <li aria-hidden="true">/</li>
          <li><Link href="/blog/use-cases" className="hover:text-fd-primary transition-colors">Use Cases</Link></li>
          <li aria-hidden="true">/</li>
      ...
      {related.length > 0 && (
        <aside className="mt-16 pt-8 border-t border-fd-border">
          <h2 className="text-xl font-semibold mb-6">More Use Cases</h2>
