...
export default async function CompatibilityPage(props: {
  ...
  const page = compatibility.getPage([params.slug]);
...
export async function generateMetadata(props: {
  ...
  const ogImage = `${baseUrl}/og/blog/compatibility/${page.slugs[0]}`;
  const dateISO = new Date(page.data.date as string).toISOString();
  const modifiedISO = page.data.dateModified
    ? new Date(page.data.dateModified as string).toISOString()
    : dateISO;
  ...
  return {
    title: `${page.data.title} | Browser Compatibility | LocalMode`,
    description: page.data.description,
    alternates: {
