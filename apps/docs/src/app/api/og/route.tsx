/**
 * @file route.tsx
 * @description Dynamic 1200×630 social-share image rendered with next/og (no extra
 * dep). Every page points `openGraph.images` / `twitter.images` at
 * `/api/og?title=…&desc=…&eyebrow=…` via `@/lib/og`, so each page gets its own
 * on-brand card. Dark, LocalMode-branded, Satori-safe (flex + inline styles only).
 */
...
export async function GET(req: Request) {
  ...
  const title = clamp(searchParams.get('title'), 'LocalMode Docs', 110);
  const description = clamp(searchParams.get('desc'), '', 180);
  const eyebrow = clamp(searchParams.get('eyebrow'), 'localmode.dev', 48);
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', fontSize: '34px', fontWeight: 800, letterSpacing: '-0.5px' }}>
            <span>LocalMode</span>
            <span style={{ color: '#71717a', fontWeight: 600 }}>&nbsp;/docs</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '24px', color: '#a1a1aa' }}>
          <span style={{ color: '#fafafa', fontWeight: 600 }}>Documentation</span>
          <span>·</span>
          <span>On-device AI, nothing leaves the browser</span>
          <span>·</span>
          <span>MIT</span>
        </div>
      </div>
