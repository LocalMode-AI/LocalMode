...
/**
 * A single collage cell: a docs-linked header over a live, mock-prop demo. The
 * demo container is registered as a static-preview root so a demo that calls
 * scrollIntoView() on mount (e.g. a cmdk list scrolling its selected item into
 * view) can't scroll the page down on load.
 */
function CollageCell({ name }: { name: string }) {
  ...
  return (
    ...
    >
      ...
      <div ref={ref} className="flex min-h-32 flex-1 items-center justify-center p-4">
...
export function LiveCollage({ items, className, cta }: LiveCollageProps) {
  return (
    <div className={cn('grid gap-4 sm:grid-cols-2 lg:grid-cols-3', className)}>
      {items.map((name) => (
        <CollageCell key={name} name={name} />
      ))}
      {cta}
    </div>
  );
